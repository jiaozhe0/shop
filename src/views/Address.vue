<template>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex == index}" @click="checkIndex=index;checkId = item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">
                      {{item.streetName}}
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="'/orderConfirm/'+ checkId">
               Next
              </router-link>
            </div>
          </div>
    ...
    <modal :mdShow="showModal" v-on:close="closeModal()">
      <p slot='message'>您确认要删除此地址？</p>
      <div slot='btnGroup'>
        <a href="javascript:;" class="btn btn--m" @click="addressDel()">确认</a>
        <a href="javascript:;" class="btn btn--m" @click="showModal=false">取消</a>
      </div>
    </modal> 
    </div>
</template>
...
<script>
  ...
  export default{
    ...
    methods: {
      getAddress () {
        axios.get('/users/address').then((respone) => {
        ...
        axios.post('/users/addressDel', {
          ...
          if (res.status === '0') {
            ...
            this.addressList = this.addressList.filter((item) => {
              return item.addressId !== this.addressId
            })
          }
        })
      },
